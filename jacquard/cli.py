...
SUBCOMMAND_GROUPS = (
    ("list", "show lists of various topics"), ("show", "show items")
)
...
def _build_argument_parser(cwd=None):
    ...
    _add_subparsers_from_plugins(
        subparsers=subparsers, plugin_group="commands"
    )
...
def _configure_process_and_load_config_from_options(
    ...
    try:
        ...
        print(
            "Could not read config file '{path}'".format(path=options.config)
        )
        sys.exit(1)
