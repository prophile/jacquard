...
class Bugpoint(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        target.add_argument("--command", type=str, help="command to run", nargs="*")
    ...
    def handle(self, config, options):
        ...
        with self._backed_up_storage(replacement_config.storage):
            ...
            self._progressively_simplify(
                replacement_config.storage, self._try_simplifying_key, predicate
            )
        ...
        if options.command:
            ...
            def target():
                ...
                with contextlib.ExitStack() as context:
                    context.enter_context(contextlib.redirect_stdout(out_stream))
                    context.enter_context(contextlib.redirect_stderr(out_stream))
            ...
            def target():
                ...
                if status_class in ("4", "5"):
                    raise ValueError("Status: {code}".format(code=result.status_code))
