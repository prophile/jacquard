...
CONTEXT = ConstraintContext(
    era_start_date=datetime.datetime(2017, 1, 1, tzinfo=UTC)
)
...
def test_constraints_forbid_users_without_required_tags():
    assert not constraint_match(
        {"required_tags": ("foo",)}, tagged_user("bar")
    )
...
def test_constraints_exclude_with_any_tags():
    assert not constraint_match(
        {"excluded_tags": ("foo", "bar")}, tagged_user("foo")
    )
